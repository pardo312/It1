...
class SQLVisitan 
{
	 ...
	 */
	public List<Producto> darVisitan (PersistenceManager pm)
	{
	 ...
	 * @return Una lista de objetos VISITAN
	 */
	private List<Producto> darVisitan_V2 (PersistenceManager pm)
	{
		Query q = pm.newQuery(SQL, "SELECT idBebedor, idBar, fechaVisita, horario FROM " + pp.darTablaVisitan ());
		List<Producto> resp = new LinkedList<>();
		...
		for (Object obj : results)
		{
			...
			resp.add (new Producto (idBebedor, idBar, fecha, horario));
		}
		return resp;		
	}
